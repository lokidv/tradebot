...
۲. **پنجرهٔ منجمد** — بازهٔ زمانیِ آزمون با مهرِ زمانیِ مطلق ثبت می‌شود و
   ``calib.build`` رویدادهای آن بازه را از **همهٔ** آموزش‌ها کنار می‌گذارد.
...
def ensure_registered(spans, now=None):
    ...
    پس هیچ مدلی رویدادهای پنجرهٔ منجمد را ندیده و داور اولین کسی است که نگاهشان
    می‌کند. اعدادِ گیت و خانوادهٔ فرضیه ثابت‌های کد هستند که پیش از هر قضاوتی
