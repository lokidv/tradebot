...
* وگرنه قاعدهٔ z/رأی: ``z ≥ 1.2`` و دست‌کم ۳ رأی از ۵ (چهار دسته + رأیِ kNN «هوش مصنوعی») و
...
def _component_votes(comp, vb=None, vs=None):
    ...
    for key, fa, series, thr in COMPONENTS:
        val = _num(comp.get(fa, comp.get(key)))
        vote = 0
        if val is not None:
...
def _reasons(d):
    ...
    if d["action"] in ("long", "short"):
        ...
        if ai and ai["vote"] == -sd:
            out.append("هوش مصنوعی (kNN) خلافِ این جهت رأی داده")
    else:
        lean = d.get("lean")
        if abs(z) >= Z_ENTRY:
            sd = 1 if z > 0 else -1
